...
async function getAudios(folder) {
  let a = await fetch(`./AudioFiles/${folder}/`);
...
function playAudio(track, pause = false) {
  currentAudio.src = `./AudioFiles/${currentFolder}/` + track;
  if (!pause) {
    currentAudio.play();
...
async function main() {
  Audios = await getAudios("Recitation");
  if (Audios.length > 0) {
  ...
  document.querySelector("#Previous").addEventListener("click", () => {
    ...
    if (index > 0) {
      currentAudio.src = `./AudioFiles/${currentFolder}/` + Audios[index - 1];
  ...
  // Adding an event listener to the volume button
  document
    .querySelector(".range")
